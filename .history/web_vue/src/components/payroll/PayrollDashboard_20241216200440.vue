...
import { exportToExcel, formatPayrollsForExport } from '../../utils/export'
  ...
  setup() {
    ...
    const handleExport = async () => {
      try {
        ...
        const filename = `payroll-${filterOptions.value.year}-${filterOptions.value.month}`
        if (!stats.value) {
    ...
    <div class="header">
      ...
      <div class="actions">
        <button 
          ...
          :disabled="exporting"
        >
          <LoadingSpinner v-if="exporting" size="small" color="#374151" />
          <span>{{ exporting ? 'Exporting...' : 'Export Payroll' }}</span>
        </button>
        <button 
