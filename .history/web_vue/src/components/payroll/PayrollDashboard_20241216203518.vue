...
import { exportToExcel, exportToCSV, formatPayrollsForExport, ExportFormat, ExportError } from '../../utils/export'
...
export default defineComponent({
  ...
  components: { ProcessPayroll, Modal, LoadingSpinner, LoadingOverlay, Line, Bar, ProgressBar },
  setup() {
    ...
    const handleExport = async (format: ExportFormat) => {
      try {
        ...
        if (format === 'excel') {
          await exportToExcel(formattedData, stats.value, filename, 
            (progress) => {
        ...
        if ((err as ExportError).type === 'excel') {
          error.value = `Failed to generate Excel file: ${(err as ExportError).details || ''}`
        } else if ((err as ExportError).type === 'csv') {
          error.value = `Failed to generate CSV file: ${(err as ExportError).details || ''}`
        } else {
          error.value = 'Failed to export payroll data'
        }
        showErrorModal.value = true
      } finally {
...
<template>
  <div class="payroll-dashboard">
    ...
    <Modal v-if="exporting && exportFormat === 'excel'" :closeable="false">
      <template #header>
        <h3>Exporting Excel File</h3>
      </template>
      <template #body>
        <ProgressBar 
        ...
        <p class="text-sm text-gray-600">
          Please wait while your file is being generated...
        </p>
      </template>
    </Modal>
  </div>
